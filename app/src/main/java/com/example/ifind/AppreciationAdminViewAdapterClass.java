...
public class AppreciationAdminViewAdapterClass extends RecyclerView.Adapter<MyViewHolderAdminAdapter> {
    private Context context;
    private List<AppreciationItemHelperClass> datalist;

    public AppreciationAdminViewAdapterClass(Context context, List<AppreciationItemHelperClass> datalist) {
        this.context = context;
        this.datalist = datalist;
    }
    @NonNull
    @Override
    public MyViewHolderAdminAdapter onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.recycler_itemappreciation1, parent, false);
        return new MyViewHolderAdminAdapter(view);
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull MyViewHolderAdminAdapter holder, int position) {
        ...
        holder.recCard.setOnClickListener(view -> {
            Intent intent = new Intent(context, AppreciationAdminView.class);
...
class MyViewHolderAdminAdapter extends RecyclerView.ViewHolder {
    ...
    public MyViewHolderAdminAdapter(@NonNull View itemView) {
        super(itemView);
